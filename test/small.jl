...
@testset "Bidirectional coloring" begin
    ...
    @testset "Anti-diagonal" begin
        ...
        result =
            coloring(A, problem, GreedyColoringAlgorithm{:direct}(; postprocessing = false))
        @test ncolors(result) == 2

        result =
            coloring(A, problem, GreedyColoringAlgorithm{:direct}(; postprocessing = true))
        @test ncolors(result) == 1

        result = coloring(
            A,
            problem,
            GreedyColoringAlgorithm{:substitution}(; postprocessing = false),
        )
        @test ncolors(result) == 2
        ...
        result = coloring(
            A,
            problem,
            GreedyColoringAlgorithm{:substitution}(; postprocessing = true),
        )
        @test ncolors(result) == 1
    end
    ...
    @testset "Rectangle" begin
        ...
        result = coloring(
            A,
            problem,
            GreedyColoringAlgorithm{:direct}(order; postprocessing = false),
        )
